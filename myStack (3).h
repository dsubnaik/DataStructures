...
public:
    ...
    ~stackType(); 
      ...
      bool operator==(const stackType& other)const;
...
template<class Type>
bool stackType<Type>::operator==(const stackType& other)const
{
  bool status=true;//stays true unless false
  //the case that the they are not the same size
  if(stackTop !=other.stackTop)
    status= false;

  else//the case that they are the same size
  {
    for(int i=0;i<stackTop;i++)
      {
        if(list[i]!=other.list[i])//checks if they are not equal
        {
          status= false;//sets to false
