...
int binaryTreeType<elemType>::internalNodesRec(binaryTreeNode<elemType> *p) const
{
  ...
  if(p==NULL)//base case
    return 0;
  else if(p->llink !=NULL && p->rlink !=NULL)//if the tree has two children then increases by 1
...
int binaryTreeType<elemType>::singleChildRec(binaryTreeNode<elemType> *p) const
{
  ...
  if(p==NULL)//base case
    return 0;
  else if(p->llink==NULL && p->rlink!=NULL)//left side of tree is single child
    return 1 + singleChildRec(p->rlink);
  else if(p->llink!=NULL && p->rlink==NULL)//right side of tree is single child
...
int binaryTreeType<elemType>::twoChildrenRec(binaryTreeNode<elemType> *p) const
{
  ...
  if(p==NULL)//base case
    return 0;
  else if(p->llink != NULL && p->rlink !=NULL)//two children case
...
int binaryTreeType<elemType>::nodeCountRec(binaryTreeNode<elemType> *p) const
{
  ...
  if(p==NULL)
    return 0;
  else//adds 1 as long as not NULL
